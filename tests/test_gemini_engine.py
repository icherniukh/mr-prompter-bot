...
async def test_remove_overlays_gemini_uses_default_prompt(monkeypatch):
    captured = {}
    ...
    out = await gemini_engine.remove_overlays_gemini(b"img", "image/png")
    assert out == b"CLEANED"
    assert captured["prompt"] == REMOVAL_PROMPT
    assert gemini_engine.REMOVAL_PROMPT == REMOVAL_PROMPT


async def test_remove_overlays_gemini_passes_custom_prompt(monkeypatch):
    captured = {}
    ...
    await gemini_engine.remove_overlays_gemini(b"img", "image/png", "custom prompt")
    assert captured["prompt"] == "custom prompt"


async def test_remove_overlays_gemini_wraps_unexpected_errors(monkeypatch):
    async def fake_to_thread(func, image_bytes, mime_type, prompt):
